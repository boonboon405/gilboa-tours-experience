...
export const Navigation = () => {
  ...
  const handleNavKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, href: string) => {
    if (e.key === 'Enter' || e.key === ' ') handleNavClick(e, href);
  };
  ...
  const navItems = [
    ...
    { label: t('nav.funDay'), href: '#choose-your-day' },
    { label: t('nav.vipTours'), href: '#vip-tours' },
    { label: t('nav.odt'), href: '#odt-section' },
    { label: t('nav.contact'), href: '#contact' },
    { label: t('nav.accessibilityStatement'), href: '/accessibility', isLink: true },
  ];
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            >
              <div className="relative">
                ...
                {quizCount > 0 && (
                  <Badge variant="secondary" className="absolute -top-3 -right-3 h-5 w-5 p-0 flex items-center justify-center text-xs animate-pulse bg-white text-primary font-bold border-2 border-primary">
                    {quizCount}
                  </Badge>
                )}
              </div>
            ...
            <Link 
              ...
              title={t('nav.accessibilityStatement')}
            >
              <Accessibility className="h-4 w-4" />
              <span className="sr-only lg:not-sr-only">{t('nav.accessibility')}</span>
            </Link>
            
            <LanguageSelector />
            
            <Link to="/booking">
              <Button variant="default" className="gap-2 hover:scale-105 transition-all duration-300 shadow-md hover:shadow-xl">
                ...
                if ((item as any).isLink) {
                  return (
                    <Link key={item.label} to={item.href} onClick={() => setIsOpen(false)}
                      className="flex items-center gap-2 py-3 px-4 rounded-lg text-foreground hover:text-primary hover:bg-primary/5 transition-all duration-300 hover:translate-x-1">
                      <Accessibility className="h-4 w-4" />
                      {item.label}
                    </Link>
                  );
                ...
                {isAdmin && (
                  <div className="mt-2 space-y-2">
                    ...
                    <Button variant="outline" className="w-full gap-2"
                      onClick={() => { navigate('/admin/knowledge'); setIsOpen(false); }}>
                      <Shield className="h-4 w-4" />
                      {t('nav.knowledgeBase')}
                    </Button>
                  </div>
